...
use ao_common::domain::dal::{Gateway, NetworkInfo, TxStatus};
...
impl ArweaveGateway {
    pub async fn new() -> Result<Self, String> {
        ...
        tokio::spawn(async move {
            loop {
                ...
                if let Ok(updated_info) = ArweaveGateway::network_info_fetch("cu".to_string()).await {
    ...
    async fn network_info_fetch(mode: String) -> Result<NetworkInfo, String> {
        let config = AoConfig::new(Some(mode)).expect("Failed to read configuration");
...
impl Gateway for ArweaveGateway {    
    async fn check_head(&self, tx_id: String) -> Result<bool, String> {
        let config = AoConfig::new(Some("cu".to_string())).expect("Failed to read configuration");
        let gateway_url = config.gateway_url;
