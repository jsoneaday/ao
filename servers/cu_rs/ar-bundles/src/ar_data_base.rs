...
pub fn get_signature_data(item: &mut DataItem) -> [u8; 48] {
    ...
    vec.append(CryptoDriver::string_to_buffer(&item.signature_type.to_string()).to_vec().as_mut());
    vec.append(item.raw_owner.as_mut());
    vec.append(item.raw_target.as_mut());
    vec.append(item.raw_anchor.as_mut());
    vec.append(item.raw_tags.as_mut());
    vec.append(item.raw_data.as_mut());
