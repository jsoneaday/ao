...
use crate::{
    ...
    bundle_item::BundleItemFn, 
    constants::SignatureConfig, 
    errors::ArBundleErrors, 
    signing::signer::SignerMaker, 
    tags::{deserialize_tags, Tag}, utils::byte_array_to_long
};
...
impl DataItem {
    ...
    fn get_target_start(&self) -> usize {
        return (2 + self.signature_length.as_ref() + self.owner_length.as_ref()) as usize;
    }

    pub fn get_raw_id(&self) -> [u8; 32] {
        sha256(self.raw_signature.as_ref())
    }
...
impl BundleItemFn for DataItem {
    ...
    fn verify<T: SignerMaker>(&self, buffer: &Vec<u8>, signer: &T) -> bool {
        ...
        signer.verify(item.raw_owner.as_ref(), &signature_data.to_vec(), item.raw_signature.as_ref());
